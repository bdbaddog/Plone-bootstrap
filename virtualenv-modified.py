...
# [newbery] Some python version checking...

if sys.version_info[:2] != (2, 4) or sys.version_info < (2, 4, 5):
    print('ERROR: %s' % sys.exc_info()[1])
    print('ERROR: This buildout requires Python 2.4, version 2.4.5 or greater.')
    sys.exit(101)
...
if majver == 2:
    ...
    print('This custom version of virtualenv does not support Python 3+')
    #parser.print_help()
    sys.exit(2)
...
def _install_req(py_executable, unzip=False, distribute=False,
    ...
    # Let's avoid writing to the current directory
    if True:
...
def main():
    ...
    create_environment(home_dir, site_packages=not options.no_site_packages, clear=options.clear,
                       ...
                       install_pip=options.install_pip,
                       install_activate=options.install_activate)
...
def create_environment(home_dir, site_packages=True, clear=False,
                       ...
                       install_pip=True, install_activate=True):
    """
...
def install_python(home_dir, lib_dir, inc_dir, bin_dir, site_packages, clear):
    ...
    # 
    # XXX - [newbery] Not sure what this is supposed to solve
    # but it breaks if using a virtualenv-based python
    # built with another user account, so we'll disable
    if False and sys.exec_prefix != prefix and not is_pypy:
        if sys.platform == 'win32':
...
def extend_parser(parser):
    ...
    parser.add_option(
        '--install-activate',
        dest='install_activate',
        action='store_true',
        help='Install activate scripts')
    
    parser.set_defaults(no_site_packages=True, use_distribute=True,
                        install_pip=False, install_activate=False,
                        clear=True)
